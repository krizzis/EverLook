...
1. **`docs/handoff.md`** → What happened last session? What's currently in progress?
2. **`docs/scope.md`** → What are we building and why? What are the success metrics?
3. **`docs/design.md`** → How is this architected? What patterns should we follow?
4. **`docs/tracker.md`** → What are the active tasks? What are their acceptance criteria?
5. **`docs/todo.md`** (if present) → What are the near-term priorities?
...
## Opening Brief
**Context Summary:** [2-3 sentences synthesizing current state from docs/handoff.md]
**Active Task:** T-### [title] — Acceptance: [copy measurable criteria from docs/tracker.md]
...
4. Update docs/handoff.md with evidence
**Questions/Assumptions:**
- Should we fail open (allow requests) or closed (deny) if Redis is down? Assuming fail-closed for security.
- Redis is already in docs/scope.md dependencies, so not adding new infra.
**Success Looks Like:**
...
```markdown
**Draft update for docs/tracker.md:**
...
- Evidence: 
  ...
  - Decision on Redis failover: docs/design.md §3.4

**Draft update for docs/handoff.md:**
[Will provide complete docs/handoff.md in Closing Report]
```
...
**Recommended approach** (based on docs/scope.md mentioning production use):
...
- Use existing logger (Winston, per docs/design.md §2.3)
...
docs/scope.md §Success Metrics says:
> "PR lead time ≤ 3 days (p50)"

But docs/tracker.md shows:
...
3. docs/scope.md SLO not yet enforced (methodology still rolling out?)
...
- C) Note SLO miss in docs/handoff.md and retrospect?
...
1. Link to the docs/tracker.md task for this work (e.g., T-042)
...
**Root cause:** I assumed bcrypt was already a dependency based on docs/design.md mentioning password hashing, but I should have checked package.json or asked you to verify.
...
**Decisions Made:** [Any significant choices + links to docs/design.md sections or PRs]
**Risks & Unknowns:** [Anything uncertain + owner + review date]
**Next Steps:** [1-3 ordered steps, each ≤1 day]

**Updated docs/handoff.md (canonical schema from docs/methodology.md §4):**
```markdown
# docs/handoff.md
...
## Decisions Made
- [decision] (link: docs/design.md §X or PR #Y)
...
- docs/docs/design.md (+18/-3): Documented rate limiting strategy (§3.4) and Redis failover decision (fail-closed)
...
**Decisions Made:**
- Rate limit set to 100 req/min with burst of 20 (balances UX vs. abuse prevention) — docs/design.md §3.4
- Redis failover: fail-closed (deny requests if Redis unavailable) for security — docs/design.md §3.4
...
4. Document blocker in docs/handoff.md → Risks & Unknowns
...
3. Recommend: update docs/scope.md/docs/design.md first, then resume
4. Document decision in docs/handoff.md
...
- [ ] docs/tracker.md updated (status, %, evidence)
- [ ] docs/handoff.md updated with canonical schema
...
- Failures must be copied and summarized in docs/handoff.md
...
**Review-only session:**
```
Load docs in SSOT order. Review latest PR diff against acceptance criteria (docs/docs/tracker.md) and quality gates (docs/methodology.md §§6-10). Provide pass/fail verdict with specific issues and fixes. No new code.
```

**Investigation/Spike session:**
```
Load docs in SSOT order. Formulate hypotheses for [problem]. Design minimal experiments with success metrics. Provide commands and expected evidence. Document findings in docs/handoff.md. No production code changes unless trivial.
```